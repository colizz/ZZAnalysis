...
SVFlavourONNXTagsProducer::SVFlavourONNXTagsProducer(const edm::ParameterSet &iConfig, const ONNXRuntime *cache)
    : src_(consumes<TagInfoCollection>(iConfig.getParameter<edm::InputTag>("src"))),
      jets_(consumes<JetCollection>(iConfig.getParameter<edm::InputTag>("jets"))),
...
void SVFlavourONNXTagsProducer::fillDescriptions(edm::ConfigurationDescriptions &descriptions) {
  ...
  desc.add<edm::InputTag>("jets", edm::InputTag("pfDeepBoostedJetTagInfos", "jets"));
...
void SVFlavourONNXTagsProducer::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  auto outJets = std::make_unique<JetCollection>();
  for (unsigned jet_n = 0; jet_n < tag_infos->size(); ++jet_n) {
    const auto &taginfo = (*tag_infos)[jet_n];
    outJets->push_back(jets->at(jet_n)); // do a copy here
    auto &jet = outJets->back();
    ...
    for (std::size_t flav_n = 0; flav_n < flav_names_.size(); flav_n++) {
      jet.addUserFloat(flav_names_[flav_n], outputs[flav_n]);
    }
  }
  ...
  if (debug_) {
    ...
    for (unsigned jet_n = 0; jet_n < tag_infos->size(); ++jet_n) {
      const auto &jet_ref = tag_infos->at(jet_n).jet();
      std::cout << " - Jet #" << jet_n << ", gen_flavour=" << outJets->at(jet_n).userFloat("gen_flavour") << std::endl;
      for (std::size_t flav_n = 0; flav_n < flav_names_.size(); ++flav_n) {
        std::cout << "    " << flav_names_.at(flav_n) << " = " << outJets->at(jet_n).userFloat(flav_names_[flav_n]) << std::endl;
      }
    }
  }
